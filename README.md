...
| `TG_WEBHOOK_SECRET_MODE` | Режим проверки секрета вебхука: `strict` (по умолчанию, отклоняет апдейт при несовпадении) или `warn` (логирует несовпадение, но продолжает обработку — для диагностики прокси/платформ). |
...
| `CREDIT_MIGRATION_ENABLED` | Включает одноразовую миграцию старых балансов при старте (умножает кредиты на `CREDITS_PER_GENERATION`). |
| `REQUEST_TIMEOUT` | Тайм-аут (в секундах) для исходящих запросов к API. |
| `DELIVERY_MODE` | Режим доставки ассетов: `default` (скачивание/файл) или `link` (отправить пользователю только ссылку на результат). |
