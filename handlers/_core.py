...
async def generate_tarot_reading(
    ...
    *,
    ...
    system_prompt = (
        "Ты – нейропомощник, который делает психологические, а не мистические "
        "расклады Таро. Используй символику карт как метафоры, помоги человеку "
        "осознать ситуацию и наметить действия. Не давай финансовых, юридических "
        "или медицинских советов. Не обещай, что что-то точно произойдёт. Пиши "
        "на русском языке в живом, дружелюбном стиле. Добавляй уместные эмодзи: "
        "для любовных раскладов – ❤️, 💕, 💌, 💋, 🫶; для карьеры и денег – 💼, 💰, 📈, 🧠, 🧾; "
    ...
    structure_instructions = """Составь интерпретацию расклада Таро по следующей структуре в Markdown:\n\n"
        ...
        "## 🧭 Советы\n"
        "Дай конкретные рекомендации, как лучше себя вести в ближайшие 1–3 месяца.\n"
        "Оформи в виде списка:\n"
        "- 💡 совет 1\n"
        "- 💡 совет 2\n"
        "- 💡 совет 3\n\n"
        "Если уместно, добавь блок:\n\n"
        "## ⚠️ Возможные риски\n"
        "Кратко опиши 2–3 момента, с которыми стоит быть осторожнее.\n"
    ...
    prompt_lines.extend(
        [
            ...
            "Эмодзи должны быть уместными, связаны с темой расклада и не должны ломать Markdown-разметку.",
        ]
    )
    prompt = "\n".join(prompt_lines)
    try:
