...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        level=_LOG_LEVEL, format=_LOG_FORMAT, datefmt="%Y-%m-%d %H:%M:%S"
    )
    ...
    def _init_application(config: Config) -> ApplicationState:
        ...
        local_app, yookassa_processor = create_app(
            config=config, dp=dp, bot=bot, db=db
        )
    ...
    async def _startup(state: ApplicationState, *, mode: str) -> None:
        ...
        try:
            migrated = await state.db.migrate_credit_balances(
                state.config.generation_cost_credits
            )
            if migrated:
    ...
    async def _run_webhook(state: ApplicationState) -> bool:
        ...
        try:
            if not _is_valid_webhook_host(CFG.WEBHOOK_HOST):
                log.error(
                    "WEBHOOK_HOST не задан или задан неверно; переключаюсь на polling"
                )
                return False
